...
- “ICMP 直连”默认开启，使 IPv4/IPv6 `ping` 走物理网卡，并在网络切换时随其他 egress 一起暂停、清空映射和恢复。ICMP 不经过 SOCKS5，且无法按应用分流；需要严格隐藏物理出口时应关闭。
...
- ICMP 直连只处理未分片 Echo Request、Echo Reply 和与请求关联的常见 ICMP 错误，不转发任意 ICMP 控制流量。它使用全局 raw socket 和临时 ID 映射，无法按 `.app` 识别，开启时所有应用的 ICMP 都会暴露物理出口。
