...
这是无需 Network Extension、无需付费 Apple Developer Program 的本机版 MacTun。普通 SwiftUI 应用负责选择程序和显示状态，应用包内的单文件 Go helper 以管理员身份创建 `utun`、管理路由并运行 tun2socks。
...
- 暂时无法确认归属的新连接会保持物理网卡直连，避免误伤未选应用；已确认属于 MacTun engine 或归属冲突的连接仍会阻断以防回环。
...
```bash
open MacTun.xcodeproj
```

选择 `MacTun` scheme 后运行即可。工程构建阶段会用系统 `go` 生成对应架构的 `mactun` helper，并放进 `MacTun.app/Contents/Resources/`。
...
2. 在 MacTun 中测试代理并添加目标 `.app`。默认保持“TCP 稳定模式”开启；需要代理 UDP 或游戏时再关闭。
3. 点击“启动 TUN”，在 macOS 管理员授权窗口中确认。
4. 使用完成后点击“停止 TUN”。应用会删除自己添加的路由并关闭 utun。

root helper 在后台运行，关闭 MacTun 窗口不会自动停止代理。也可以在终端中检查或停止：

```bash
/path/to/MacTun.app/Contents/Resources/mactun status
...
- `SIGKILL` 或断电无法执行即时清理；下次启动会恢复残留状态，也可以运行 `sudo mactun down`。
