	...
	for {
		...
		case eventErr := <-stopEventCh:
			if eventErr != nil {
				...
				fmt.Fprintln(a.out, "received tunscope down request, restoring routes...")
			}
			break activeLoop
...
func (a *App) recoverWindowsStale() error {
	...
	if processAlive(state.OwnerPID) {
		...
		if identityErr == nil && matches {
			return fmt.Errorf("tunscope state belongs to a still-running owner PID %d", state.OwnerPID)
		}
...
func (a *App) Down() error {
	...
	if err != nil {
		...
		return fmt.Errorf("verify tunscope owner PID %d: %w", state.OwnerPID, err)
	}
	if !matches {
		return fmt.Errorf("tunscope owner PID %d identity changed; refusing to signal a reused PID", state.OwnerPID)
	}
...
func (a *App) windowsStateForLockedOwner(timeout time.Duration) (*State, error) {
	...
	for {
		...
		if time.Now().After(deadline) {
			return nil, fmt.Errorf("tunscope lock/state owner identity is inconsistent")
		}
		time.Sleep(25 * time.Millisecond)
	}
}
...
func (a *App) Status() error {
	...
	fmt.Fprintf(a.out, "phase: %s\nproxy: %s\ndevice: %s\nphysical interface: %s\n", report.Phase, report.Proxy, report.Device, report.Interface)
	...
	if targets := report.Applications + report.PackageFamilies; targets > 0 {
