...
static int mactun_snapshot_flows(const int *pids, int pid_count, mactun_flow **result) {
	int capacity = 256;
	int count = 0;
	mactun_flow *flows = (mactun_flow *)calloc((size_t)capacity, sizeof(mactun_flow));
	...
	for (int p = 0; p < pid_count; p++) {
		...
		for (int i = 0; i < fd_count; i++) {
			...
			if (count == capacity) {
				capacity *= 2;
				mactun_flow *grown = (mactun_flow *)realloc(flows, (size_t)capacity * sizeof(mactun_flow));
				if (grown == NULL) {
					free(fds);
					free(flows);
					return -1;
...
func snapshotDarwinProcesses(previous map[int]cachedProcess) (darwinProcessSnapshot, error) {
	...
	var raw *C.mactun_flow
	count := 0
	if len(pids) > 0 {
		count = int(C.mactun_snapshot_flows((*C.int)(unsafe.Pointer(&pids[0])), C.int(len(pids)), &raw))
	}
	if count < 0 {
...
func convertDarwinFlow(item C.mactun_flow) (darwinFlow, bool) {
...
func darwinAddresses(item C.mactun_flow) (netip.Addr, netip.Addr, bool) {
