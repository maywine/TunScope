...
func TestCleanupWithoutProcessHandleTerminatesEngine(t *testing.T) {
	if os.Getenv("MACTUN_CLEANUP_HELPER") == "1" {
	...
	cmd.Env = append(os.Environ(), "MACTUN_CLEANUP_HELPER=1")
...
func TestKernelLockHelper(t *testing.T) {
	if os.Getenv("MACTUN_LOCK_HELPER") != "1" {
		return
	}
	if err := acquireLock(); err != nil {
		t.Fatal(err)
...
func TestKernelLockCannotBeReleasedByContender(t *testing.T) {
	...
	cmd.Env = append(os.Environ(), "MACTUN_LOCK_HELPER=1")
