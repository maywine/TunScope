...
// CLI `tunscope up` remains a foreground command with Ctrl-C handling.
func runDetachedUpLauncher(args []string, stdout, stderr io.Writer) int {
	if os.Geteuid() != 0 {
		fmt.Fprintln(stderr, "tunscope launcher: administrator privileges are required")
		return 1
	}
	exe, err := os.Executable()
	if err != nil {
		fmt.Fprintf(stderr, "tunscope launcher: locate executable: %v\n", err)
		return 1
	}
	...
	if err := configureDetachedProcess(cmd); err != nil {
		fmt.Fprintf(stderr, "tunscope launcher: configure detached owner: %v\n", err)
		return 1
	}
	if err := cmd.Start(); err != nil {
		fmt.Fprintf(stderr, "tunscope launcher: start detached owner: %v\n", err)
		return 1
	}
	pid := cmd.Process.Pid
	if err := cmd.Process.Release(); err != nil {
		fmt.Fprintf(stderr, "tunscope launcher: release detached owner PID %d: %v\n", pid, err)
		return 1
	}
	fmt.Fprintf(stdout, "launcher: started detached tunscope owner PID %d at %s\n", pid, time.Now().Format(time.RFC3339))
	return 0
}
...
func printUsage(w io.Writer) {
	fmt.Fprint(w, `tunscope - lightweight TUN mode for macOS
...
Run "tunscope up" and "tunscope down" with sudo. Press Ctrl-C to stop and restore routes.
`)
}
...
func runEngineChild() int {
	f := os.NewFile(3, "tunscope-engine-config")
	if f == nil {
	...
	commandFile := os.NewFile(4, "tunscope-engine-commands")
	responseFile := os.NewFile(5, "tunscope-engine-responses")
	...
	var perAppDialer *tunscope.PerAppDialer
	if len(cfg.Applications) > 0 {
		var err error
		perAppDialer, err = tunscope.NewPerAppDialer(
			cfg.Proxy,
			cfg.Applications,
			cfg.ProxyUDP,
			cfg.TrustedDNS,
