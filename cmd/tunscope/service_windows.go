...
const (
	windowsServiceName        = "TunScope"
	windowsServiceDisplayName = "TunScope Per-Application Proxy"
...
func (h *windowsServiceHandler) Execute(_ []string, requests <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	logFile := h.logFile
	if logFile == nil {
		logFile = func() (io.WriteCloser, error) { return tunscope.OpenWindowsServiceLog() }
	}
	...
	if run == nil {
		run = func(stop <-chan struct{}, onActive func(), out, errOut io.Writer) error {
			cfg, err := tunscope.LoadWindowsServiceConfig()
			if err != nil {
				return fmt.Errorf("load %s: %w", tunscope.WindowsServiceConfigPath(), err)
			}
			return tunscope.New(out, errOut).UpAsWindowsService(cfg, stop, onActive)
		}
	}
...
func configureWindowsService(args []string, stdout, stderr io.Writer) error {
	...
	var cfg tunscope.Config
	var err error
	if *fromStdin {
		cfg, err = tunscope.DecodeWindowsServiceConfig(os.Stdin)
	} else {
		cfg = tunscope.DefaultConfig()
		err = loadConfig(*configPath, &cfg)
		if err == nil {
			cfg, err = tunscope.NormalizeWindowsServiceConfig(cfg)
		}
	}
	if err != nil {
		return err
	}
	if err := tunscope.SaveWindowsServiceConfig(cfg); err != nil {
		return err
	}
	fmt.Fprintf(stdout, "service config saved securely: %s\n", tunscope.WindowsServiceConfigPath())
...
func installWindowsService(startup string, stdout, stderr io.Writer) error {
	...
	if err := tunscope.EnsureWindowsServiceStorage(); err != nil {
		return err
	}
...
type windowsServiceStatus struct {
	...
	Runtime     tunscope.WindowsRuntimeStatus `json:"runtime"`
}

func queryWindowsServiceStatus() (windowsServiceStatus, error) {
	result := windowsServiceStatus{
		State:      "not-installed",
		ConfigPath: tunscope.WindowsServiceConfigPath(),
		LogPath:    tunscope.WindowsServiceLogPath(),
		Runtime:    tunscope.ReadWindowsRuntimeStatus(),
	}
	if _, err := tunscope.LoadWindowsServiceConfig(); err == nil {
...
func cleanStaleWindowsServiceRuntime(stdout, stderr io.Writer) error {
	...
	case "stale":
		...
		if err := tunscope.New(stdout, stderr).Down(); err != nil {
			return fmt.Errorf("clean stale Windows Service runtime: %w", err)
		}
		if remaining := tunscope.ReadWindowsRuntimeStatus(); remaining.Status != "stopped" {
			return fmt.Errorf("TUN runtime is still %s after cleanup: %s", remaining.Status, remaining.Detail)
		}
		return nil
	case "active":
		return fmt.Errorf("SCM reports the Windows Service stopped, but another TunScope owner is active; refusing to stop or delete it")
	default:
...
func startWindowsService(service *mgr.Service, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		...
		if status.State == svc.Stopped {
			...
			return fmt.Errorf("Windows Service stopped during startup with exit code %d; see %s", exitCode, tunscope.WindowsServiceLogPath())
		}
		time.Sleep(250 * time.Millisecond)
	}
	...
	for time.Now().Before(deadline) {
		...
		case svc.Stopped:
			if stopSent {
				...
				if exitCode != 0 {
					return fmt.Errorf("Windows Service stopped with exit code %d; cleanup may be incomplete, see %s", exitCode, tunscope.WindowsServiceLogPath())
				}
			}
			return nil
