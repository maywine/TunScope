...
```powershell
$archive = '.\tunscope-0.3.22-windows-amd64.zip'
...
解压后保持 `TunScope.exe`、`tunscope-cli.exe` 和 `wintun.dll` 位于同一目录，直接双击 `TunScope.exe` 并确认 UAC 即可。GUI 内的“启动连接”“停止连接”和“保存并重启”直接控制前台数据面，不需要执行 PowerShell 脚本，也不会注册 Windows Service。
...
```powershell
Set-Location .\tunscope-0.3.22-windows-amd64
...
```bash
make windows-amd64 VERSION=0.3.22
make windows-gui VERSION=0.3.22
```
...
dotnet publish .\gui\TunScope.GUI.csproj `
  ...
  -p:Version=0.3.22 -o .\bin\windows-gui
.\windows\package.ps1 `
...
`package.ps1` 默认执行二进制的 `version` 命令取得包版本，也可显式传入 `-Version 0.3.22`。打包时会拒绝 CLI、GUI 和指定包版本不一致的产物；GUI 状态卡会显示自身构建版本。脚本还会重新下载并校验固定版本的官方 Wintun 归档，然后生成 ZIP 和 ZIP 的 SHA-256 文件。

维护者推送形如 `v0.3.22` 或 `v0.3.22-rc.1` 的标签时，`release-windows.yml` 会在真实 Windows runner 上测试、注入标签版本、打包，并创建或更新 GitHub Release。标签版本不会依赖源码里的默认开发版本。
